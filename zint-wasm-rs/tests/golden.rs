...
fn option_cases() -> Vec<Case> {
    ...
    vec![
        ...
        // A fixed Data Matrix size, which is what `dm-size` returns: 27 is the
        // 12x26 entry of zint's size table and 28 the 12x36 one. Both are here
        // because they differ only in width, so naming one where the other is
        // meant draws a symbol that looks entirely reasonable on its own.
        Case::new(
            "option-2-data-matrix-size-12x26",
            Symbology::DataMatrix,
            DATA,
        )
        .option_2(27),
        Case::new(
            "option-2-data-matrix-size-12x36",
            Symbology::DataMatrix,
            DATA,
        )
        .option_2(28),

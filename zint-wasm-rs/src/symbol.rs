...
use crate::{
    error::{Error, ZintResult}, options::{output_options::OutputOptions, symbology::Symbology, DisplayOptions, GenericOptions}, output::PlotResult, segment::{Segment, ECI}
};
...
impl Symbol {
    pub fn encode_ascii<'o>(options: impl Into<GenericOptions<'o>>, text: &str) -> Result<Self, Error> {
    ...
    pub fn encode_utf8<'o>(options: impl Into<GenericOptions<'o>>, text: &str) -> Result<Self, Error> {
        let options = options.into();
        let eci = if options.supports_eci() {
            ECI::UTF_8
        } else {
    ...
    pub fn encode_segments<'o>(options: impl Into<GenericOptions<'o>>, data: &[Segment<'_>]) -> Result<Self, Error> {
        let options = options.into();
        
        // SAFETY: Segment is a transparent wrapper of zint_seg
        let segments: &[zint_seg] = unsafe {
            std::mem::transmute::<&[Segment::<'_>], &[zint_seg]>(data)
        };
        ...
        let result = ZintResult::from(unsafe {
            ZBarcode_Encode_Segs(result_ptr, segments, segment_count as i32) as u32
        }, result_ptr);
        match result {
    ...
    fn plot_raster_data(&self, options: &DisplayOptions) -> Result<(), Error> {
        let result = ZintResult::from(unsafe { ZBarcode_Buffer(self.inner, options.rotation.into()) as u32 }, self.inner);
    ...
    fn plot_vector_data(&self, options: &DisplayOptions) -> Result<(), Error> {
        let result = ZintResult::from(unsafe { ZBarcode_Buffer_Vector(self.inner, options.rotation.into()) as u32 }, self.inner);
    ...
    pub fn plot<'a, R: PlotResult<'a>>(&'a mut self, options: &'a DisplayOptions) -> Result<R, Error> {
